...
bot_token = '6572750464:AAEG4X_rOQ3SrfRMsBsTwTzrWLcdh5FjJtI'  # Thay YOUR_BOT_TOKEN bằng mã token của bot Telegram của bạn
bot = telegram.Bot(token=bot_token)

allowed_group_id = -1001916749047 # Thay YOUR_GROUP_ID bằng ID của nhóm bạn muốn bot hoạt động trong đó

allowed_users = []
processes = []
ADMIN_ID = 6127851200  # Thay 123456789 bằng ID của admin
...
def khanh(update, context):
    ...
    if update.message.chat_id != allowed_group_id:
        update.message.reply_text('Bot chỉ hoạt động trong nhóm này https://t.me/spamsmsfreev9.')
        return
    ...
    if phone_number in ['113', '114', '0376349783', '0333079921', '0974707985', '0915215448', '+84397333616', '+84915215448', '+84974707985', '0978551717', '116', '911', '0961639461']:
    ...
    if phone_number in last_used_times:
        ...
        if current_time - last_used_time < 300:
            ...
            update.message.reply_text(f"Bạn hãy spam số khác, Số {phone_number} đang trong quá trình tấn công. Vui lòng chờ {remaining_time} giây mới tấn công lại số này.")
            return

    user_mention = update.message.from_user.mention_html()
    hi = f'''=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
┣➤ ➡️  Telegram Users : ➡️ {user_mention} ✅
┣➤ 1️⃣ Spam calls thành công ✅
┣➤ 2️⃣ SDT : {phone_number} ✅ 
┣➤ 3️⃣ Thời gian : Tấn công {spam_time} phút spam theo yêu cầu 🕢
┣➤ 4️⃣ Đây Là Thông Báo Hoàn thành do người dùng sử dụng
┣➤ 5️⃣ Muốn Dùng Gõ /how ✅
┣➤ Bạn Đang Sử Dụng Tools Free ✅
...
def button(update, context):
    ...
    if query.data == 'momo':
        momo_message = f'''liên hệ admin để mua
Họ tên chủ tài khoản: lưu anh hoàng
tele https://t.me/hoangnocode'''
        context.bot.send_message(chat_id=chat_id, text=momo_message)

    elif query.data == 'mbbank':
        mbbank_message = f'''liên hệ admin để mua
Họ tên chủ tài khoản: lưu anh hoàng
tele https://t.me/hoangnocode'''
...
def plan(update, context):
    reply_text = 'Giá cả của các gói dịch vụ tất cả đều chát admin @hoangnocode:\n\n'
    ...
    if len(text) >= 2 and text[0].strip() == "/key":
        ...
        if key == "":
            ...
            if key == generate_key(encoded_user_id):
                ...
                group_id = "-1001961518812"  # Replace with your actual group ID
...
def get_key(update, context):
    ...
    if update.message.chat.type == 'private':
        ...
        long_url = f"http://anhgit.site/key={key}"
        api_token = '9b878167-88cc-455b-915f-c88a3dc18fbd'
        ...
        update.message.reply_text(f"Link key của bạn là: {link} Sau khi bạn vượt link thành công, hãy sử dụng lệnh [/key keyhomnay] để xác thực key.")
    else:
        # Nếu là tin nhắn từ nhóm, gửi thông báo yêu cầu nhắn riêng
        user_id = update.effective_user.id
        button_text = "Nhắn tin riêng để nhận key"
        button_url = f"https://t.me/devsorrysnbot?start={user_id}"
...
def main():
    ...
    start_handler = CommandHandler('hoitro', how_to)
    dispatcher.add_handler(start_handler)

    start_handler = CommandHandler('start', how_to)
    dispatcher.add_handler(start_handler)
    ...
    how_handler = CommandHandler('how', how_to)
